...
- 拡張機能で作成したタイムスタンプ、設定情報、画像情報

これにより、拡張機能の次回利用時などに情報が過去に作成したデータを自動で復元することができ、利便性が向上します。
**個人情報は当開発者に送信されず拡張機能利用者のブラウザのローカルストレージに保存されます**

##### Google Drive連携機能（オプション）

拡張機能では、利用者が任意で有効化できるGoogle Drive連携機能を提供しています。

**利用目的：**

- データのバックアップ（Google Driveへの保存・復元）

**データの取り扱い：**

- 利用者が明示的に認証・有効化した場合のみ動作します
- 機能を有効化すると、データ作成・更新時に自動的にGoogle Driveに保存されます
- データは利用者のGoogle Driveアカウント内にのみ保存され、当開発者はアクセスできません
- データの復元は利用者の操作によってのみ実行されます
...
拡張機能において、情報の漏洩、滅失を防止するために以下の措置を講じています。

- 利用目的外のサーバーへの情報送信を行いません
- 送信されたデータは利用者のGoogle Driveアカウント内にのみ保存され、第三者（当開発者を含む）はアクセスできません
