...
## モバイルアプリケーションのプライバシーポリシー

本プライバシーポリシーは、Takanori Azegami（以下、「当開発者」）が開発したモバイルアプリケーション（Android 版および iOS 版の OSHIStamp）（以下、「アプリ」とします）の利用において、利用者の個人情報もしくはそれに準ずる情報を取り扱う際に、当開発者が遵守する方針を示したものです。
...
本アプリは、動画視聴時のタイムスタンプを作成・管理するためのツールです。本アプリには、Google AdMob によるバナー広告が表示されます。

#### 3.1 アプリの権限および端末アクセス

本アプリは、プラットフォームや利用する機能に応じて、以下の権限または端末アクセスを使用します：

- アプリ内ストレージ：タイムスタンプデータや設定情報を保存するために使用します
- バックアップデータのエクスポート・インポート時のファイルアクセス：利用者が明示的にバックアップまたは復元を行う場合にのみ使用します
...
#### 4.4 Google Drive 連携機能（オプション）

本アプリでは、利用者が任意で有効化できる Google Drive 連携機能を提供しています。

**利用目的：**

- データのバックアップ（Google Drive への保存）

**データの取り扱い：**

- 利用者が明示的に認証・有効化した場合のみ動作します
- 機能を有効化すると、データ作成・更新時に自動的に Google Drive に保存されます
- データは利用者の Google Drive アカウント内のアプリ専用バックアップフォルダにのみ保存され、当開発者はアクセスできません
...
ただし、AdMob 広告に関しては、Google により一部情報が自動的に収集される場合があります。詳細は「4.6 AdMob によるデータ収集」をご覧ください。
